...
<div class="row">
    <?php echo e(Form::open(['method' => 'GET', 'route' => ['pdf.collections_deposits'], 'id' => 'form_print'])); ?>

        <div class="form-group col-sm-4">
            <label for="type">Type</label>
            <select class="form-control" name="type" required autofocus>
                <option selected disabled></option>
                <?php foreach($base['categories'] as $category): ?>
                  <!-- exclude BTS, effective 2020 --> 
                  <?php /* <?php if($category->id != 2): ?> */ ?>
        ...
        <div class="form-group col-sm-4">
            ...
            ?>
            <input type="text" class="form-control" name="report_no" value="<?php echo e($report_no); ?>" required>
        </div>
        <div class="form-group col-sm-4">
            <label for="report_no">Report DATE</label>
            <input type="text" class="form-control date" name="report_date" value="<?php echo e(date('m/d/Y')); ?>" required>
        </div>
            ...
            <select class="form-control" name="report_officer" id="report_officer_name">
              ...
              <?php
                  foreach ($base['report_officers'] as $officer) { ?>
                    <option value="<?php echo e($officer['officer_id']."_".$officer['position_id']."_".$officer['position']); ?>"><?php echo e($officer['officer_name']); ?></option>
              <?php   }
              ?>
            </select>
        </div>
        ...
        <div class="form-group col-sm-12">
            <button type="submit" class="btn btn-primary pdf generate" name="button_pdf" id="confirm">EXPORT TO PDF</button>
            <button type="submit" class="btn btn-primary generate" name="button_excel" id="confirm">EXPORT TO EXCEL</button>
            <button type="submit" class="btn btn-primary generate" name="button_html" id="confirm">Generate Report HTML</button>
        </div>

        <div class="form-group col-sm-12">
            <button type="submit" class="btn btn-info pdf generate" name="button_pdf_type" value="type_A" id="confirm">EXPORT TO PDF A</button>
            <button type="submit" class="btn btn-info pdf generate" name="button_pdf_type"  value="type_B"  id="confirm">EXPORT TO PDF B</button>
            <button type="submit" class="btn btn-info pdf generate" name="button_pdf_type"  value="type_C"  id="confirm">EXPORT TO PDF CD</button>
          <br /> <br />
            <button type="submit" class="btn btn-info generate" name="button_pdf_type" value="type_html_A" id="confirm">Generate Report HTML A</button>
...
<?php if( \Session::get('user')->position == 'Administratorx' ): ?>
<h4>ALLOWED MONTHS FOR PROVINCIAL INCOME</h4>
    <?php echo e(Form::open(['method' => 'POST' , 'route' => ['allow_mnths.collections_deposits']])); ?>


    <div class="form-group">
            <label>YEAR</label>
            <input type="number" class="form-control" name="monthly_allowed_prvncial_year" value="<?php echo e(date('Y')); ?>" readonly>
    </div>
