...
<!-- <div class="footer">
 ...
 <div class="col-sm-12">
 <h4><strong>DATE :  <?php echo e($data['start_date']->format('D F d, Y')); ?> <?php echo e(($data['start_date']!=$data['end_date'] ) ? '- '. $data['end_date']->format('D F d, Y'):''); ?> Provinical Income</strong></h4>
                        ...
                        if( $asdate[$x]['D'] != 'Sun'  && $asdate[$x]['D'] != 'Sat' ){
                            ...
                            if( $title->id == '55'){
                                  ...
                                  $title_total= count($title_total)>0 ? $title_total : [] ;
                                    ...
                                    $title_total = $title->receipt()->where('col_receipt.report_date','LIKE',$asdate[$x]['y-m-d'].'%' )->where('col_receipt.is_cancelled','=','0')->where('col_receipt.is_printed','=','1')->select(["col_receipt_items.share_provincial","col_receipt_items.value"])->get();
                                    $title_total= count($title_total)>0 ? $title_total: [] ;
                                    $titlereceipt_div_value = 0;
                                    foreach($title_total as $receipt_value){
                                        if($receipt_value->share_provincial > 0){
                                            $titlereceipt_div_value = $titlereceipt_div_value + $receipt_value->share_provincial;
                                        }else{
                                            if($title->id == $insurance_premium){
                                                ...
                                                $titlereceipt_div_value +=  $receipt_value->share_provincial;
                                            } 
                                        }
                            ...
                            foreach ($title->subs as $subs){
                                ...
                                for( $y = 0 ; $y<=$data['diff']; $y++){
                                        if( $asdate[$y]['D'] != 'Sun'  && $asdate[$y]['D'] != 'Sat' ){
                                                ...
                                                $subtitle_total = $subs->receipt()->where('col_receipt.report_date','LIKE',$asdate[$y]['y-m-d'].'%' )->where('col_receipt.is_cancelled','=','0')->where('col_receipt.is_printed','=','1')->select(["col_receipt_items.share_provincial","col_receipt_items.value"])->get();
                                                $subtitle_total= count($subtitle_total)>0 ? $subtitle_total: [] ;
                                                $subsreceipt_div_value = 0;
                                                foreach($subtitle_total as $receipt_value){
                                                    if($receipt_value->share_provincial >0){
                                                        $subsreceipt_div_value += $subsreceipt_div_value  + $receipt_value->share_provincial;
                                                    }else{
                                                        $subsreceipt_div_value += $receipt_value->share_provincial;
                                                    }
...
<?php foreach($categories as $category): ?>
 <?php if(isset($pm_categ[$category->id])): ?>
    <tr class="page-break">
                <td colspan="<?php echo e($colsx +2); ?>" ><?php echo e($category->name); ?></td>
    </tr>
    <?php foreach($category->group as $group): ?>
    <?php if(isset($pm_categ[$category->id][$group->id])): ?>
        <tr class="page-break">
                    <td colspan="<?php echo e($colsx +2); ?>" style="padding: 1px 1px 1px 30px;" ><?php echo e($group->name); ?></td>
        </tr>
        <?php $totalxxt = []; ?>
        <?php foreach($group->title as $title): ?>
        <?php if(in_array($title->id,$pm_categ[$category->id][$group->id])): ?>
        <?php $totalxxt[$category->id][$group->id][$title->id] = 0; ?>
            <tr class="page-break">
                            ...
                            <?php for( $x = 0 ; $x<=$data['diff']; $x++): ?>
                                ...
                                <?php endif; ?>
                            <?php endfor; ?>
                            <td  class="text-right"> <?php echo e(number_format($totalxxt[$category->id][$group->id][$title->id] , 2)); ?></td>
            </tr>
            <?php foreach($title->subs as $subs): ?>
                <tr class="page-break">
                                ...
                                <?php for( $y = 0 ; $y<=$data['diff']; $y++): ?>
                                    ...
                                    <?php endif; ?>
                                <?php endfor; ?>
                                <td  class="text-right"> <?php echo e(number_format($subtitle_total_sum[$group->id][$title->id][$subs->id] , 2)); ?></td>
                </tr>

            <?php endforeach; ?><!-- end subtitle -->
            <?php endif; ?>
        <?php endforeach; ?><!-- end title -->
        <?php endif; ?>
    <?php endforeach; ?><!-- end group -->
<?php endif; ?>
