...
<div class="row">
    ...
    <?php echo e(Form::open(['method' => 'POST', 'route' => ['pdf.accounts_report']])); ?>

        <div class="form-group col-sm-12">
            <label for="report_no">Report Number</label>
            <input type="text" class="form-control" name="report_no" value="<?php echo e(date('Y')); ?>" required>
        </div>
...
<!-- <div class="row">
    ...
    <?php echo e(Form::open(['method' => 'POST', 'route' => ['pdf.per_accounts_report']])); ?>

    <div class="form-group col-sm-6">
            <label for="report_no">ACCOUNT</label>
            <select name="account" class="form-control">
                <?php foreach($base['titles'] as $keyt => $title): ?>
                    <option value="<?php echo e($title->id); ?>"><?php echo e($title->name); ?> ( <?php echo e($title->group->category->name); ?> ) </option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="form-group col-sm-6">
...
<div class="row">
    <h4>PER-ACCOUNT REPORT </h4>
    <?php echo e(Form::open(['method' => 'POST', 'route' => ['pdf.per_accounts_report2'], 'target' => '_blank'] )); ?>
...
<script>
    ...
    $.fn.checkSharing = function(){
        ...
        if(account){
            $.ajax({
              type: 'POST',
              url: '<?php echo e(route("report.checkSharing")); ?>',
              data: {
                account : account,
                subtitle : subtitle,
    ...
    $.fn.getSubs = function(){
        ...
        if(account){
            ...
            $.ajax({
              type: 'POST',
              url: '<?php echo e(route("report.accounts_report_getsubs")); ?>',
              data: {
                account : account,
                _token : '<?php echo e(csrf_token()); ?>',
              },
