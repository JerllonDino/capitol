...
//for ada
<?php if(isset($base['ada_settings'])): ?>
    var ada_bank_name = '<?php echo e($base['ada_settings'][0]->value); ?>';
    var ada_bank_number = '<?php echo e($base['ada_settings'][1]->value); ?>';
<?php else: ?>
    var ada_bank_name = '';
    var ada_bank_number = '';
<?php endif; ?>
...
function handle_rate(response, shared_acc, element, id, type) {
    ...
    if (!response[0]) {
        ...
        var sum_given = '';
        var deadline = '';
        var deadline_datex = "<?php echo e(date('m/d/Y')); ?>";
        var with_surviving = '';
        ...
        if (response[0].pct_deadline == 1) {
            if(response[0].pct_deadline_date != null){
                 deadline_datex = response[0].pct_deadline_date+"/<?php echo e(date('Y')); ?>";
            }
...
$("#form").on('change', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            var serial_id = '<?php echo e(Session::get('serial_id')); ?>';
...
$.fn.getCurrrentReceipt = function(serial_id){
        $.ajax({
            type: 'POST',
            url: '<?php echo e(route("serial.get_serial_current")); ?>',
            data: {
                '_token': '<?php echo e(csrf_token()); ?>',
                'serial_id': serial_id
            },
...
$.fn.natureAutoComplete = function(){
     $('.nature').autocompleteX({
        serviceUrl: '<?php echo e(route("collection.autocomplete")); ?>',
        dataType: 'json',
        type: 'POST',
        params : {
                _token : '<?php echo e(csrf_token()); ?>',
