...
<form class="form-inline">
  <div class="form-group">
    ...
    <input type="number" min="2017" max="<?php echo e($yr); ?>" class="form-control" id="show_year" placeholder="<?php echo e(date('Y')); ?>" value="<?php echo e(date('Y')); ?>">
  </div>
  <div class="form-group">
...
$.fn.loadTable = function(){
  if ( $.fn.DataTable.isDataTable('#seriallist') ) {
    ...
    $('#seriallist').dataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: { 'url' : '<?php echo e(route("collection.datatables", "field_land_tax")); ?>',
                ...
                function(data) {
                    ...
                    if(data.col_receipt_serial_parent != null ){
                        if(data.col_receipt_serial_parent == data.serial_no ){
                            cert = (data.is_cancelled == 0) ? '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'" class="btn btn-sm btn-red datatable-btn" title="'+data.cert_typex+' Certificate"><i class="fa fa-certificate"></i></a>' : '';
                        }
                    }else{
                        var cert_type = '';
                        if(data.rcpt_certificate != null) {
                            switch(data.rcpt_certificate.col_rcpt_certificate_type_id) {
                                case 1 : cert_type = 'Provincial Permit'; break;
                                case 2 : cert_type = 'Transfer Tax'; break;
                                case 3 : cert_type = 'Sand & Gravel'; break;
                                case 4 : cert_type = 'Sand and Gravel Certification'; break;
                                default: cert_type = '';
                            }
                        }
                        if(data.with_cert != null) {
                            if(data.with_cert.cert_type === 'Transfer Tax' || cert_type === 'Transfer Tax'){
                                cert_color = 'bg-cert-ttc';
                            }else if(data.with_cert.cert_type === 'Sand & Gravel' || cert_type === 'Sand & Gravel'){
                                cert_color = 'bg-cert-sg';
                            }else if( data.with_cert.cert_type === 'Provincial Permit' || cert_type === 'Provincial Permit'){
                                cert_color = 'bg-cert-pp';
                            }

                            if(data.with_cert.process_status == '0'){
                                cert = (data.is_cancelled == 0) ? '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'" class="btn btn-sm btn-gray datatable-btn" title="'+data.with_cert.cert_type+' Certificate"><i class="fa fa-certificate"></i></a>' : '';
                            }else if(data.with_cert.process_status == '1'){
                                cert = (data.is_cancelled == 0) ? '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'" class="btn btn-sm '+cert_color+' datatable-btn" title="'+data.with_cert.cert_type+' Certificate"><i class="fa fa-certificate"></i></a>' : '';
                            }else if(cert_type != ''){
                                cert = (data.is_cancelled == 0) ? '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'" class="btn btn-sm '+cert_color+' datatable-btn" title="'+cert_type+' Certificate"><i class="fa fa-certificate"></i></a>' : '';
                            }else{
                                cert = (data.is_cancelled == 0) ? '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'" class="btn btn-sm btn-gray datatable-btn" title="Certificate"><i class="fa fa-certificate"></i></a>' : '';
                            }
                        } else if(data.rcpt_certificate != null) {
                            if(cert_type === 'Transfer Tax'){
                                cert_color = 'bg-cert-ttc';
                            }else if(cert_type === 'Sand & Gravel'){
                                cert_color = 'bg-cert-sg';
                            }else if(cert_type === 'Provincial Permit'){
                                cert_color = 'bg-cert-pp';
                            }
                    ...
                    view = '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'" class="btn btn-sm btn-info datatable-btn" title="View"><i class="fa fa-eye"></i></a>';
                    <?php endif; ?>

                    <?php if( Session::get('permission')['col_field_land_tax'] & $base['can_write'] ): ?>
                    write = (data.is_cancelled == 0) ? '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'/edit" class="btn btn-sm btn-info datatable-btn" title="Edit"><i class="fa fa-pencil-square-o"></i></a>' : '';
                    <?php endif; ?>

                    // if(data.process_status == '0'){

                    // }else if(data.process_status == '1'){
                    //     cert = (data.is_cancelled == 0) ? '<a href="<?php /* route('receipt.index') */ ?>/'+data.id+'/certificate?types=field" class="btn btn-sm btn-green datatable-btn" title="'+data.cert_type+' Certificate"><i class="fa fa-certificate"></i></a>' : '';
                    // }else if(data.col_rcpt_certificate_type_id != null){
                    //     cert = (data.is_cancelled == 0) ? '<a href="<?php /* route('receipt.index') */ ?>/'+data.id+'/certificate?types=field" class="btn btn-sm btn-green datatable-btn" title="'+data.cert_typex+' Certificate"><i class="fa fa-certificate"></i></a>' : '';
                    // }else{
                    //     cert = (data.is_cancelled == 0) ? '<a href="<?php /* route('receipt.index') */ ?>/'+data.id+'/certificate?types=field" class="btn btn-sm btn-gray datatable-btn" title="Certificate"><i class="fa fa-certificate"></i></a>' : '';
                    // }

                     <?php if( Session::get('permission')['col_receipt'] & $base['can_write'] ): ?>
                       if(data.is_printed == 1){
                            if(data.col_receipt_serial_parent == null)
                                another = (data.is_cancelled == 0) ? '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'/another?types=field" class="btn btn-sm btn-another-none datatable-btn" title="ANOTHER RECEIPT"><i class="fa fa-plus"></i></a>' : '';
                            else
                                if(data.col_receipt_serial_parent == data.serial_no ){
                                another = (data.is_cancelled == 0) ? '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'/another?types=field" class="btn btn-sm btn-another datatable-btn" title="PARENT : '+data.col_receipt_serial_parent+'"><i class="fa fa-plus"></i></a>' : '';
                                }else{
                    ...
                    <?php if( Session::get('permission')['col_receipt'] & $base['can_write'] ): ?>
                       if(data.is_printed == 1){
                            if(data.col_receipt_serial_parent == null){
                                ...
                                another = (data.is_cancelled == 0) ? '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'/another" class="btn btn-sm btn-another-none datatable-btn" title="ANOTHER RECEIPT"><i class="fa fa-plus"></i></a>' : '';
                            }else{
                                if(data.col_receipt_serial_parent == data.serial_no ){
                                    ...
                                    another = (data.is_cancelled == 0) ? '<a href="<?php echo e(route('field_land_tax.index')); ?>/'+data.id+'/another" class="btn btn-sm btn-another datatable-btn" title="PARENT : '+data.col_receipt_serial_parent+'"><i class="fa fa-plus"></i></a>' : '';
                                }else{
        ...
        swal({
            ...
            preConfirm: (receiptxx) => {
                return new Promise((resolve) => {
                  setTimeout(() => {
                    $.ajax({
                            url: '<?php echo e(route("combine.field_land_tax_combine")); ?>',
                            type: 'POST',
                            data:{
