...
<div class="row">
    {{ Form::open(['method' => 'POST', 'route' => ['opag.store']]) }}
 ...
 $.fn.loadTable = function(){
  ...
  $('#seriallist').DataTable().destroy();
    ...
    $('#seriallist').dataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: '{{ route("collection.datatables", "opag") }}',
        columns: [
            ...
            { data:
                function(data) {
                    ...
                    @endif
                    @if ( Session::get('permission')['col_cash_division'] & $base['can_write'] )
                    write = '<a href="{{ route('opag.index') }}/'+data.id+'/edit" class="btn btn-sm btn-info datatable-btn" title="Edit"><i class="fa fa-pencil-square-o"></i></a>';
                    @endif
                    @if ( Session::get('permission')['col_cash_division'] & $base['can_write'] )
