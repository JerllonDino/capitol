...
<div class="row">
    ...
    <div class="form-group col-sm-12">
        ...
        <input type="hidden" class="form-control" name="transaction_source" id="transaction_source" value="opag">
    </div>
    <div class="row">
        ...
        <table class="table" id="table">
            <thead>
                <tr>
                    ...
                    <th>Amount</th>
                    <th><button id="add_row" class="btn btn-sm btn-success" type="button" data-transactionType="2"><i class="fa fa-plus"></i></button></th>
                </tr>
            </thead>
            <tfoot>
                <tr>
...
$.fn.loadTable = function(){
  if ( $.fn.DataTable.isDataTable('#seriallist') ) {
    ...
    $('#seriallist').dataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: { 'url' : '{{ route("collection.datatables", "opag") }}',
           ...
           { data: 'date_of_entry',name : 'date_of_entry',
            ...
            { data:
                function(data) {
                    ...
                    view = '<a href="{{ route('opag.index') }}/'+data.id+'" class="btn btn-sm btn-info datatable-btn" title="View"><i class="fa fa-eye"></i></a>';


                    write = (data.is_cancelled == 0) ? '<a href="{{ route('opag.index') }}/'+data.id+'/edit" class="btn btn-sm btn-info datatable-btn" title="Edit"><i class="fa fa-pencil-square-o"></i></a>' : '';
                    ...
                    // }
                       ...
                       if(data.is_printed == 1){
                            if(data.col_receipt_serial_parent == null)
                                another = (data.is_cancelled == 0) ? '<a href="{{ route('opag.index') }}/'+data.id+'/another?types=field" class="btn btn-sm btn-another-none datatable-btn" title="ANOTHER RECEIPT"><i class="fa fa-plus"></i></a>' : '';
                            else
                                if(data.col_receipt_serial_parent == data.serial_no ){
                                another = (data.is_cancelled == 0) ? '<a href="{{ route('opag.index') }}/'+data.id+'/another?types=field" class="btn btn-sm btn-another datatable-btn" title="PARENT : '+data.col_receipt_serial_parent+'"><i class="fa fa-plus"></i></a>' : '';
                                }else{
                       ...
                       if(data.is_printed == 1){
                            if(data.col_receipt_serial_parent == null){
                                ...
                                another = (data.is_cancelled == 0) ? '<a href="{{ route('opag.index') }}/'+data.id+'/another" class="btn btn-sm btn-another-none datatable-btn" title="ANOTHER RECEIPT"><i class="fa fa-plus"></i></a>' : '';
                            }else{
                                if(data.col_receipt_serial_parent == data.serial_no ){
                                    ...
                                    another = (data.is_cancelled == 0) ? '<a href="{{ route('opag.index') }}/'+data.id+'/another" class="btn btn-sm btn-another datatable-btn" title="PARENT : '+data.col_receipt_serial_parent+'"><i class="fa fa-plus"></i></a>' : '';
                                }else{
