...
<html>
<head>
    <title>ACCOUNTS REPORT Provinical Income</title>
 ...
 <h4><strong>DATE :  {{$data['start_date']->format('D F d, Y')}} {{ ($data['start_date']!=$data['end_date'] ) ? '- '. $data['end_date']->format('D F d, Y'):'' }}</strong></h4>
        ...
        @foreach ($categories as $category)
         <?php $category_total[$category->id] = 0; ?>
            <tr class="page-break">
                <td colspan="{{$data['diff']+3}}" ><div class="col-sm-12"><b>{{ $category->name }}</b></div></td>
            </tr>
            @foreach ($category->group as $group)
             <?php $group_total[$group->id] = 0; ?>
                <tr class="page-break">
                    <td colspan="{{$data['diff']+3}}" ><div class="col-sm-12">{{ $group->name }}</div></td>
                </tr>
                            ...
                            <tr class="page-break">
                                ...
                                @for( $x = 0 ; $x<=$data['diff']; $x++)
                                    ...
                                    <?php
                                        ...
                                        $subtitle_total = $subs->receipt()->where('col_receipt.report_date','LIKE',$asdate[$x]['y-m-d'].'%' )->where('col_receipt.is_cancelled','=','0')->where('col_receipt.is_printed','=','1')->select(["col_receipt_items.share_provincial","col_receipt_items.value"])->get();
              ...
              <?php   $category_total[$category->id] = $category_total[$category->id] + $group_total[$group->id] ; ?>
                ...
                <td class="total_group"><div class="col-sm-12"><strong>{{ $group->name }} TOTAL</strong></div></td>
                 <td  colspan="{{$data['diff']+2}}" class="text-right total_group" >{{ number_format($group_total[$group->id],2) }}</td>
            </tr>
            @endforeach <!-- end of groups -->
                  <tr class="page-break">
                <td class="total_categ"><strong>{{ $category->name }} TOTAL</strong></td>
                 <td colspan="{{$data['diff']+2}}" class="text-right total_categ" >{{ number_format($category_total[$category->id],2) }}</td>
            </tr>

            @if($tr_counter / 20 === 1)
                 <tr class="page-break-before">
                    <td colspan="{{$data['diff']+2}}" >TEST</td>
                </tr>
            @endif
