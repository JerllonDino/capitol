...
@foreach ($categories as $category)
    <tr class="page-break">
                <td colspan="{{ $colsx +2}}" ><strong>{{ $category->name }}</strong></td>
    </tr>
    @foreach ($category->group as $group)
        <tr class="page-break">
                    <td colspan="{{ $colsx + 2}}" style="padding: 1px 1px 1px 30px;" ><strong>{{ $group->name }}</strong></td>
        </tr>
        <?php $totalxxt = []; ?>
        @foreach($group->title as $title)
        <?php $totalxxt[$category->id][$group->id][$title->id] = 0; $dt_t = 0; ?>
            <tr class="page-break">
                            ...
                            @for( $x = 0 ; $x<=$data['diff']; $x++)
                                ...
                                @endif
                            @endfor
                            <td  class="text-right gtotal"> {{ number_format($totalxxt[$category->id][$group->id][$title->id] , 2) }}</td>
            </tr>
            ...
            @foreach ($title->subs as $subs)
                <tr class="page-break">
                                ...
                                @for( $y = 0 ; $y<=$data['diff']; $y++)
                                    ...
                                    @endif
                                @endfor
                                <td  class="text-right gtotal"> {{ number_format($subtitle_total_sum[$group->id][$title->id][$subs->id] , 2) }}</td>
                </tr>
...
<tr>
    ...
    @foreach($date_total as $date_k => $dt_value)
        <td class="text-right gtotal">{{ number_format($dt_value,2) }}</td>
    @endforeach
