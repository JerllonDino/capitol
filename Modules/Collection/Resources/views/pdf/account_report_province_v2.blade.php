...
<!-- <div class="footer">
 ...
 <div class="col-sm-12">
 <h4><strong>DATE :  {{$data['start_date']->format('D F d, Y')}} {{ ($data['start_date']!=$data['end_date'] ) ? '- '. $data['end_date']->format('D F d, Y'):'' }} Provinical Income</strong></h4>
...
@foreach ($categories as $category)
 @if(isset($pm_categ[$category->id]))
    <tr class="page-break">
                <td colspan="{{ $colsx +2}}" >{{ $category->name }}</td>
    </tr>
    @foreach ($category->group as $group)
    @if(isset($pm_categ[$category->id][$group->id]))
        <tr class="page-break">
                    <td colspan="{{ $colsx +2}}" style="padding: 1px 1px 1px 30px;" >{{ $group->name }}</td>
        </tr>
        <?php $totalxxt = []; ?>
        @foreach($group->title as $title)
        ...
        <?php $totalxxt[$category->id][$group->id][$title->id] = 0; ?>
            <tr class="page-break">
                            <td style="padding: 1px 1px 1px 50px;">{{ $title->name }}</td>
                            ...
                            @for( $x = 0 ; $x<=$data['diff']; $x++)
                                @if( $asdate[$x]['D'] != 'Sun'  && $asdate[$x]['D'] != 'Sat' )
                                    ...
                                    ?>                                       
                                     <td class="text-right">{{ number_format($title_totlasss,2) }}</td>
                                @endif
                            @endfor
                            <td  class="text-right"> {{ number_format($totalxxt[$category->id][$group->id][$title->id] , 2) }}</td>
            </tr>
            @foreach ($title->subs as $subs)
                <tr class="page-break">
                                <td style="padding: 1px 1px 1px 70px;"> {{ $subs->name }}</td>
                                @for( $y = 0 ; $y<=$data['diff']; $y++)
                                    @if( $asdate[$y]['D'] != 'Sun'  && $asdate[$y]['D'] != 'Sat' )
                                        <td class="text-right">{{ number_format($total_subtitle[$category->id][$group->id][$title->id][$subs->id][$asdate[$y]['y-m-d']],2) }}</td>
                                    @endif
                                @endfor
                                <td  class="text-right"> {{ number_format($subtitle_total_sum[$group->id][$title->id][$subs->id] , 2) }}</td>
                </tr>

            @endforeach<!-- end subtitle -->
            @endif
        @endforeach<!-- end title -->
        @endif
    @endforeach<!-- end group -->
@endif
