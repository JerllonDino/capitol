                ...
                @else
                        ...
                        @foreach ($receipt->F56Detailmny as $f56_detail)
                            @if($f56_detail->period_covered >= \Carbon\Carbon::now()->addYear()->format('Y') || strpos('advance', $f56_detail->period_covered) != false)
                                <?php
                                    $counter++; 
                                    // advance values
                                    $adv_val = $f56_detail->tdrp_assedvalue*.01;
                                    $adv_discount = ($f56_detail->tdrp_assedvalue*.01)*.10; // fixed, 10% discount for the ff. year
                                    $adv_gross = $adv_val;
                                ...
                                <tr style="background-color: ##5af542;">
                                    ...
                                    @if(!isset($f56_detail->TDARP[0]))
                                        ...
                                        <td class="border_all">{{ isset($receipt->brgy_name) ? $receipt->brgy_name : '' }}</td>
                    ...
                    @else
                        <?php
                            $counter++; 
                            // advance values
                            $adv_val = $receipt->tdrp_assedvalue*.01;
                            $adv_discount = ($f56_detail->tdrp_assedvalue*.01)*.10; // fixed, 10% discount for the ff. year
                            $adv_gross = $adv_val;
                            $adv_net = $adv_val - $adv_discount;
                            $adv_val_total += $adv_val;
            ...
            @foreach($receipts as $receipt)
                ...
                @foreach($receipt->F56Detailmny as $f56_detail)
                    <?php 
                        ...
                        $tax_due += $f56_detail->tdrp_assedvalue*.01;
                    ?>
                @endforeach
            @endforeach
            <?php
