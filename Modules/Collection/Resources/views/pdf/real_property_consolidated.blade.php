...
<head>
    ...
    <style type="text/css">
        html {
         ...
         /* class works for the font of the table */ 
        .table-bordered.collection{
...
<body>
    ...
    <table>
        ...
        <tr>
			...
			<td><h3> A. COLLECTIONS </h3></td>
			<td></td>
            
        </tr>
        
    </table>
    <table class="table table-bordered collection">
        <thead>
        <tr>
            ...
            @endphp
                ...
                ?>
                    ...
                    <tr>
                        @if ($rcpt_done == 0)
                            <?php   $rcpt_done = 1; ?>
                            <td class="border_all" rowspan="">
                                {{-- date('M d', strtotime($receipt->report_date)) --}}
                                {{ date('M d', strtotime($receipt->date_of_entry)) }}
                            </td>
                            <td class="border_all"  rowspan="">{{ $receipt->customer->name }}</td>
                            <td class="border_all"   rowspan="">{{ $f56_detail->period_covered }}</td>
                            <td class="border_all"  rowspan="" >{{ $receipt->serial_no }}</td>
                        @else
                            <td></td>
                            <td></td>
                            <td></td>
