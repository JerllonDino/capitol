...
<div class="footer">
 ...
 <div class="col-sm-12">
 <h4><strong>DATE :  {{$data['start_date']->format('D F d, Y')}} {{ ($data['start_date']!=$data['end_date'] ) ? '- '. $data['end_date']->format('D F d, Y'):'' }} Provinical Incomex</strong></h4>
        ...
        foreach ($group->title as $title){
            ...
            for( $x = 0 ; $x<=$data['diff']; $x++){
                        if( $asdate[$x]['D'] != 'Sun'  && $asdate[$x]['D'] != 'Sat' ){
                            ...
                            $title_total = $title->receipt()->where('col_receipt.report_date','LIKE',$asdate[$x]['y-m-d'].'%' )->where('col_receipt.is_cancelled','=','0')->where('col_receipt.is_printed','=','1')->select(["col_receipt_items.share_municipal","col_receipt_items.value"])->get();
                            ...
                            foreach ($title->subs as $subs){
                                ...
                                for( $y = 0 ; $y<=$data['diff']; $y++){
                                        if( $asdate[$y]['D'] != 'Sun'  && $asdate[$y]['D'] != 'Sat' ){
                                                ...
                                                $subtitle_total = $subs->receipt()->where('col_receipt.report_date','=',$asdate[$y]['y-m-d'] )->where('col_receipt.is_cancelled','=','0')->where('col_receipt.is_printed','=','1')->select(["col_receipt_items.share_municipal","col_receipt_items.value"])->get();
                                                $subtitle_total= count($subtitle_total)>0 ? $subtitle_total: [] ;
                                                $subsreceipt_div_value = 0;
                                                foreach($subtitle_total as $receipt_value){
                                                    if($receipt_value->share_municipal >0){
                                                        $subsreceipt_div_value += $subsreceipt_div_value  + $receipt_value->share_municipal;
                                                    }else{
                                                        $subsreceipt_div_value += $receipt_value->share_municipal;
                                                    }
...
@foreach ($categories as $category)
 @if(isset($pm_categ[$category->id]))
    <tr class="page-break">
                <td colspan="{{ $data['diff'] == 0 ? 3 :  $data['diff']-1 }}" ><div class="col-sm-12"><b>{{ $category->name }}</b></div></td>
    </tr>
    @foreach ($category->group as $group)
    @if(isset($pm_categ[$category->id][$group->id]))
        <tr class="page-break">
                    <td colspan="{{  $data['diff'] == 0 ? 3 :  $data['diff']-1 }}" ><div class="col-sm-12">{{ $group->name }}</div></td>
        </tr>
        <?php $totalxxt = []; ?>
        @foreach($group->title as $title)
        ...
        <tr class="page-break">
                <td class="total_group"><div class="col-sm-12"><strong>{{ $group->name }} TOTAL</strong></div></td>
                 <td  colspan="{{$data['diffx']-1}}" class="text-right total_group" ></td>
            </tr>
        @endif
