...
<div class="row">
    <div class="col-md-12">
        ...
        <a class="btn btn-info hidden" href="{{route('pdf.land_tax_collection',['nsign',$base['receipt']->id])}}">Print w/o Signatories</a>
           @if ( Session::get('permission')['col_field_land_tax'] & $base['can_write'] )
                    <a href="{{ route('form56.view','') }}/{{ $base['receipt']->id }}" class="btn  btn-info datatable-btn" title="Edit"><i class="fa fa-eye"></i></a>
            @endif
  ...
  {{ Form::open(['method' => 'POST', 'route' => ['form56.update',$base['receipt_id']]]) }}
            <input type="hidden" name="isEdit" id="isEdit" value="isEdit">
            <dl class="dl-horizontal">
            <dt>User</dt>
            <dd>{{ $base['receipt']->user->realname }}
                <input type="hidden" name="receipt_id" id="receipt_id" value="{{ $base['receipt_id'] }}" >
            </dd>
            <dt>AF Type</dt>
            ...
            <dd>
                 <div class="form-group col-sm-4">
                    <input type="text" class="form-control" name="customer" id="customer" value="{{ $base['receipt']->customer->name }}" required>
                    <input type="hidden" name="customer_id" id="customer_id" value="{{ $base['receipt']->customer->id }}" >
                </div>
            ...
            {{ $base['receipt']->serial->municipality->name }}
                <input type="hidden" name="municipality" id="municipality" value="{{ $base['receipt']->serial->municipality->id }}" data-code="{{ $base['receipt']->serial->municipality->code }}" >
            @endif
            </dd>
            <!-- <dt>Barangay</dt>
            <dd> -->
            ...
            <dd>
                <div class="form-group col-sm-4">
                    <input type="text" class="form-control datepicker" name="date" id="date_timex" value="{{ date('m/d/Y  H:i:s', strtotime($base['receipt']->date_of_entry)) }}"  required autofocus>
                </div>
            </dd>
            <!-- <dt>Report Date</dt>
            <dd>
                <div class="form-group col-sm-4">
                    <input type="text" class="form-control datepicker" name="report_date" id="date_timex" value="{{-- date('m/d/Y', strtotime($base['receipt']->report_date)) --}}"  required autofocus>
                </div>
            </dd> -->
                     ...
                     <div class="form-group col-sm-4">
                        <select class="form-control" id="transaction_type" name="transaction_type" required>
                            ...
                            @foreach ($base['transaction_type'] as $transaction_type)
                                @if($base['receipt']->transactiontype->id == $transaction_type->id )
            ...
            <dd>
                <div class="form-group col-sm-4">
                    <input type="text" class="form-control bank_input datepicker" id="bank_date" name="bank_date" value="{{ $base['receipt']->bank_date }}" >
                </div>
            </dd>
            <?php
            ...
            <dd>
                <div class="form-group col-sm-4">
                    <select class="form-control" name="previous_tax_type">
                        @foreach($tax_types as $i => $type)
                            @if(!is_null($base['receipt_tdarp']))
                                @if($i == $base['receipt_tdarp']->previous_tax_type_id)
    ...
    @endphp
     <div class="form-group col-sm-4">
        ...
        <input type="text" class="form-control" id="prev_receipt_no" name="prev_receipt_no" value="{{ $base['receipt']->F56Previuos->col_receipt_no ?? 'x' }}" >
    </div>
    <div class="form-group col-sm-4">
        <label for="bank_date">Prev-Date</label>
        <input type="text" class="form-control datepicker2" id="prev_date" name="prev_date" value="{{ $base['receipt']->F56Previuos->col_receipt_date ?? '' }}" >
    </div>

    <div class="form-group col-sm-4">
        <label for="prev_for_the_year">For the YEAR</label>
        <input type="text" class="form-control"  id="prev_for_the_year" name="prev_for_the_year" value="{{ $base['receipt']->F56Previuos->col_receipt_year ?? '' }}" >
    </div>

    <div class="form-group col-sm-12">
        <label>Previous Receipt Remarks</label>
        <textarea type="text" name="prev_remarks" rows="2" class="form-control" value="{{ $base['receipt']->F56Previuos->col_prev_remarks ?? '' }}"></textarea>
    </div>
</div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="hidden" name="f56_detail_id[]" id="f56_detail_id[]" value="{{ $F56Detail->id }}" >
                        <input type="hidden" class="form-control tdarpno_id" name="tdarpno_id[]" value="{{ $F56Detail->TDARPX ? $F56Detail->TDARPX->id : ''}}" required>
                        <input type="text" class="form-control declared_owner" name="declared_owner[]" value="{{$F56Detail->owner_name ??   ''}}" required>

                    </td>
                    <td>
                        <input type="text" class="form-control tdarpno" name="tdarpno[]" value="{{$F56Detail->TDARPX ?  $F56Detail->TDARPX->tdarpno : ''}}" required>
                    </td>


                    <td>
                        <select class="form-control tdrp_barangay" name="tdrp_barangay[]" id="tdrp_barangay[]"  >
                              @foreach ($base['barangay'] as $brgy)
                                @if($F56Detail->TDARPX && $F56Detail->TDARPX->barangay == $brgy->id )
                                    <option data-code="{{ $brgy->code }}" value="{{ $brgy->id }}" selected>{{ $brgy->name }}</option>
                                @else
                                    <option data-code="{{ $brgy->code }}" value="{{ $brgy->id }}">{{ $brgy->name }}</option>
                                @endif
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <select class="form-control f56_type" id="f56_type[]" name="f56_type[]" required>
                            ...
                            @foreach ($base['f56_types'] as $type)
                                @if($F56Detail->TDARPX &&  $F56Detail->TDARPX->f56_type == $type->id )
                    ...
                    <td><input type="text" class="form-control tdrp_assedvalue" value="{{ ($F56Detail->tdrp_assedvalue) }}" name="tdrp_assedvalue[]"  name="tdrp_assedvalue[]" ></td>
                    ...
                    <td><input type="text" class="form-control period_covered" name="period_covered[]" value="{{ ($F56Detail->period_covered) }}" required></td>
                    <td>
                        ...
                        @if(!is_null($F56Detail->ref_num))
                            <div id="ref_num_input">
                                ...
                                @elseif($F56Detail->full_partial == 8)
                                ...
                                @endif
                                <input class="form-control" type="number" id="ref_num" name="ref_num[]" value="{{ $F56Detail->ref_num }}"></input>
                            </div>
                        @endif
                    </td>
                    <td><input type="number" class="form-control basic_current" name="basic_current[]" value="{{ ($F56Detail->basic_current) }}" min="0" step="0.01" required></td>
                    <td><input type="number" class="form-control basic_discount" name="basic_discount[]" value="{{ ($F56Detail->basic_discount) }}" min="0" step="0.01" required></td>
                    <td><input type="number" class="form-control basic_previous" name="basic_previous[]" value="{{ ($F56Detail->basic_previous) }}" min="0" step="0.01" required></td>
                    <td><input type="number" class="form-control basic_penalty_current" name="basic_penalty_current[]" value="{{ ($F56Detail->basic_penalty_current) }}" min="0" step="0.01" required></td>
                    <td><input type="number" class="form-control basic_penalty_previous" name="basic_penalty_previous[]" value="{{ ($F56Detail->basic_penalty_previous) }}" min="0" step="0.01" required></td>
                    ...
                    <td class="sefxx">{{ $sef }}</td>
                    <td class="basicxx">{{ $sef }}</td>
                    <td class="grand_total_net">{{ $sef*2 }}</td>
                     <td>
                        @if($key > 0)
   ...
   <table class="table">
    ...
    <tbody>
        ...
        <input type="hidden" name="account_id[]" value="{{$item->col_acct_title_id}}">
        ...
        <tr>
            ...
            <td><input type="text" class="form-control" name="nature[]" required="required" maxlength="300" value="{{ $item->nature }}" ></td>
            <td align="right"><input type="number" min="0" step=".01" class="form-control amounts" name="amount[]" required="required" value="{{ $item->value }}"> </td>
        </tr>
        @endforeach
    </tbody>
...
@section('js')
    @include('collection::form56/form56_rules')
    @include('collection::form56/js')
