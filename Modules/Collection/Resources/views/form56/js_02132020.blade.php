...
$('.tdarpno').change(function(){
    ...
    if($('.tdarpno').eq(el_index).val() != ''){
            $.ajax({
              url: hostss+'/capitol_rpt/public/api/bms_get_tax_dec_details', // LOCAL LINK /capitol_rpta, please change to capitol_rpt
              type: 'POST',
              data:{
      ...
      $.ajax({
        ...
        'success': function(data) {
          ...
          if(data != null && data != undefined && data != "") {
            ...
            if(data.f56_detail != null && data.f56_detail != undefined)
              $('#prev_for_the_year').val(data.f56_detail.period_covered);

            // var receipts = [];
            // $.each(data, function(key, node) {
            //   receipts.push(node.serial_no);
            // });
            // $($('#prev_receipt_no').target).find('#prev_receipt_no').autocomplete();
            // if ($('#prev_receipt_no').hasClass('ui-autocomplete')) {
            //   // $('#prev_receipt_no').autocomplete('destroy');
            //   console.log('set')
            // }
            // $.fn.prevno_autocomplete(receipts);
          } else {
            $('#prev_rcpt_info').html('No previous Form 56 transaction found');
          }
        }
      });
    });
