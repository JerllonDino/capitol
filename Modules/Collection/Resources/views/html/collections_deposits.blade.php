...
<tfoot>
   ...
   <div class="col-md-6">
             <table class="table table-condensed">
                    ...
                    @if ($base['bac_type_1'] > 0 && $_GET['type'] == 1)
                    <tr >
                        <td>BAC Goods & Services</td>
                        <td class="val">
                            <span class="hidden">
                            {{ $total += $base['bac_type_1'] }}
                            </span>
                            {{ number_format($base['bac_type_1'], 2) }}
                        </td>
                    </tr>
                    @endif

                    @if ($base['bac_type_2'] > 0 && $_GET['type'] == 1)
                    <tr >
                        <td>BAC INFRA</td>
                        <td class="val text-right">
                            <span class="hidden">
                            {{ $total += $base['bac_type_2'] }}
                            </span>
                            {{ number_format($base['bac_type_2'], 2) }}
                        </td>
                    </tr>
                    @endif

                    @if ($base['bac_type_3'] > 0 && $_GET['type'] == 1)
                    <tr >
                        <td>BAC Drugs & Meds</td>
                        <td class="val text-right">
                            <span class="hidden">
                            {{ $total += $base['bac_type_3'] }}
                            </span>
                            {{ number_format($base['bac_type_3'], 2) }}
                        </td>
                    </tr>
                    @endif
    ...
    <table class="table table-no-border table-condensed">
        ...
        <td  >
            ...
            <table class="table">
                ...
                @foreach ($base['amusement_shares'] as $i => $share)
                    <tr>
                        <td><b>{{ $share['name'] }}</b></td>
                        <td class="val text-right">
                            <span class="hidden">
...
<h4>B. REMITTANCES/DEPOSITS</h4>
        ...
        <tr >
            <td class="">
                ...
                @elseif($_GET['type'] == 2)
                    ...
                    @endif
                @else
                    @if(isset($base['officer_name']->value))
                        {{ $base['officer_name']->value }}
                    @else 
                        {{ $base['officer_name'] }}
                    @endif
                @endif
    ...
    <tbody>
        @foreach ($base['rcpt_acct'] as $rcpt)
        <tr class="page-break">
            <td class="">
                <span class="hidden">
    ...
    <tr>
        ...
        <td class="table-border-right">
            <table class="table table-condensed">
    			<tr>
    				<td class="">Beginning Balance {{ $base['report_start'] }}</td>
    				<td class=" val"></td>
                    <td class=" val text-right">0.00</td>
    			</tr>
    			<tr>
    				<td class="">Add: Collections {{ $base['date_range'] }}</td>
    				<td class=" val"></td>
                    <td></td>
    			</tr>
            ...
            <tr>
                <td class="">
                    <table class="table table-no-border">
                        ...
                        <tr >
                            <th class="border-botts">{{ $base['acctble_officer_name']->value }}</th>
                            <th></th>
                            <th class="border-botts">{{ date('F d, Y') }}</th>
                        </tr>
                        <tr>
                            <th colspan="2">{{ $base['acctble_officer_position']->value }}</th>
                            <th>Date</th>
                        </tr>
                    </table>
                </td>
                ...
                <td class="">
                    <table class="table table-no-border">
                        <tr>
                            ...
                            <b><u>{{ $base['total_in_words'] }}</u> (PHP {{ number_format($summary_total - $bank_depo, 2) }})</b>.
                            </td>
                        </tr>
                        <tr><td colspan="3">&nbsp;</td></tr>
                        <tr >
                        ...
                        <tr>
                            <th>
                                @if ($_GET['type'] == 2) 
                                    ...
                                    @endif
                                @else
                                    @if(isset($base['officer_position']->value))
                                        {{ $base['officer_position']->value }}
                                    @else 
                                        {{ $base['officer_position'] }}
                                    @endif
                                @endif
                            </th>
                            <th></th>
