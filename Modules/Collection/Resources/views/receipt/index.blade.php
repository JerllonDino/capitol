                ...
                function(data) {
                    ...
                    if(data.col_receipt_serial_parent != null ){
                                if(data.col_receipt_serial_parent == data.serial_no ){
                                    cert = (data.is_cancelled == 0) ? '<a href="{{ route('receipt.index') }}/'+data.id+'/certificate" class="btn btn-sm btn-red datatable-btn" title="'+data.cert_typex+' Certificate"><i class="fa fa-certificate"></i></a>' : '';
                                }
                    }else{
                        ...
                        if(data.process_status == '0'){
                            cert = (data.is_cancelled == 0) ? '<a href="{{ route('receipt.index') }}/'+data.id+'/certificate" class="btn btn-sm btn-gray datatable-btn" title="'+data.cert_typex+' Certificate"><i class="fa fa-certificate"></i></a>' : '';
                        }else if(data.process_status == '1'){
                             cert = (data.is_cancelled == 0) ? '<a href="{{ route('receipt.index') }}/'+data.id+'/certificate" class="btn btn-sm '+cert_color+' datatable-btn" title="'+data.cert_typex+' Certificate"><i class="fa fa-certificate"></i></a>' : '';
                        }else if(data.col_rcpt_certificate_type_id != null){
                             cert = (data.is_cancelled == 0) ? '<a href="{{ route('receipt.index') }}/'+data.id+'/certificate" class="btn btn-sm '+cert_color+' datatable-btn" title="'+data.cert_typex+' Certificate"><i class="fa fa-certificate"></i></a>' : '';
                        }else{
                            cert = (data.is_cancelled == 0) ? '<a href="{{ route('receipt.index') }}/'+data.id+'/certificate" class="btn btn-sm btn-gray datatable-btn" title="Certificate"><i class="fa fa-certificate"></i></a>' : '';
                        }
                    ...
                    write = (data.is_cancelled == 0) ? '<a href="{{ route('receipt.index') }}/'+data.id+'/edit" class="btn btn-sm btn-info datatable-btn" title="Edit"><i class="fa fa-pencil-square-o"></i></a>' : '';
                    @endif
                     ...
                     @if ( Session::get('permission')['col_receipt'] & $base['can_write'] )
                       if(data.is_printed == 1){
                            if(data.col_receipt_serial_parent == null){
                                ...
                                another = (data.is_cancelled == 0) ? '<a href="{{ route('receipt.index') }}/'+data.id+'/another" class="btn btn-sm btn-another-none datatable-btn" title="ANOTHER RECEIPT"><i class="fa fa-plus"></i></a>' : '';
                            }else{
                                if(data.col_receipt_serial_parent == data.serial_no ){
                                    //combine = '<button class="btn btn-sm btn-warning datatable-btn" title="Combine" onclick="$(this).combine(\''+data.id+'\');" ><img src="{{-- asset('asset/images/arrows_converge_2-512.png') --}}" class="img-responsive combinex" /></a>';
                                    ...
                                    another = (data.is_cancelled == 0) ? '<a href="{{ route('receipt.index') }}/'+data.id+'/another" class="btn btn-sm btn-another datatable-btn" title="PARENT : '+data.col_receipt_serial_parent+'"><i class="fa fa-plus"></i></a>' : '';
                                }else{
            ...
            swal({
                  ...
                  preConfirm: (receiptxx) => {
                    return new Promise((resolve) => {
                      setTimeout(() => {
                        $.ajax({
                                url: '{{ route("combine.check_serial_combine") }}',
                                type: 'POST',
                                data:{
