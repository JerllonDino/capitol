    ...
    <thead>
        <tr class="page-break">
            ...
            @foreach($accounts as $i => $account)
                <th rowspan="1" colspan="{{ count($account['titles']) + count($account['subtitles']) }}">{{ $i }} </th>
            @endforeach

            @if (count($shares) > 0)
            <th rowspan="1" colspan="{{ $share_columns }}">MUNICIPAL & BRGY SHARES</th>
            @endif
        ...
        <tr>
            ...
            @foreach($shares as $i => $share)
                <th rowspan="2" >{{ $share['name'] }}</th>
                @foreach($share['barangays'] as $j => $barangay)
                <th rowspan="2" >{{ $barangay['name'] }}</th>
                @endforeach
            @endforeach
        </tr>
</thead>
