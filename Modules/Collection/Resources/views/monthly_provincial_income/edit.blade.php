...
@section('content')
<div class="row">
    {{ Form::open([ 'method' => 'post', 'route' => ['monthly_provincial_income.update', $base['year'], $base['month']] ]) }}
                        ...
                        <td>
                            ...
                            ?>
                                @if ( $title_mnthly_prov_incme )
                                     <input class="form-control" type="number" step="0.01" name="title_value[]" value="{{ $title_mnthly_prov_incme->value }}">
                         ...
                         <td>
                            <div class="col-sm-12">
                                ...
                                <?php $total_value = $title_mnthly_prov_incme->reconciliation_value + $title_mnthly_prov_incme->value ; ?>
                                     <input class="form-control title_reconciliation" type="number" step="0.01" name="title_reconciliation[]" value="{{ $title_mnthly_prov_incme->reconciliation_value }}">
                                @else
                                    <input class="form-control title_reconciliation" type="number" step="0.01" name="title_reconciliation[]" value="0">
                                @endif
                            </div>
                        </td>
                        <td>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @if ( $substitle_mnthly_prov_incme )
                                    <input class="form-control" type="number" step="0.01" name="subtitle_value[]" value="{{ $substitle_mnthly_prov_incme->value }}">
...
<script type="text/javascript">
    ...
    $('.title_reconciliation').on('change',function(){
            var el = $(this);
            var el_index = $('.title_reconciliation').index(el);
            var acct_val = $('input[name="title_value[]"]').eq(el_index).val();
            var total_value = parseFloat(acct_val) + parseFloat(el.val());
            $('input[name="total_value[]"]').eq(el_index).val(total_value);
    });

    $('.subtitle_reconciliation').on('change',function(){
            var el = $(this);
            var el_index = $('.subtitle_reconciliation').index(el);
            var acct_val = $('input[name="subtitle_value[]"]').eq(el_index).val();
            var total_value = parseFloat(acct_val) + parseFloat(el.val());
            $('input[name="subtotal_value[]"]').eq(el_index).val(total_value);
    });
</script>
@endsection