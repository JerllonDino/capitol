...
<script>
    $('#example').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: '{{ route("collection.datatables", "subtitleitems") }}',
        columns: [
            { data: 'item_name', name: 'item_name' },
            { data: 'name', name: 'col_acct_subtitle.name' },
            { data: null , name : '',
                render : function(data) {
                    ...
                    can_delete = '<a href="{{ route('account_subtitle_items.index') }}/'+data.id+'/destroy" class="btn btn-sm btn-danger datatable-btn" title="Delete"><i class="fa fa-trash"></i></a>';
                    @endif
    ...
    @if ( Session::get('permission')['col_settings'] & $base['can_write'] )
        $("div.dt-custom").html(
            '<a href="{{ route("account_subtitle_items.create") }}" class="btn btn-med btn-success">Add</a>'
        );
    @endif
</script>
@endsection
