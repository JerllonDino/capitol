...
use Modules\Collection\Entities\{CollectionRate,AccountTitle,AccountSubtitle,BudgetEstimate,AccountCategory,AllowedMonths,Municipality,CashDivision,CashDivisionItems,Receipt,ReceiptItems,Customer};

use PDF,Carbon\Carbon,Excel;
   ...
   public function __construct(Request $request)
    ...
    public function index(){
        ...
        for ($month = 1; $month <= 12; $month++) {
            ...
            return view('collection::accounts.report', $this->base)->with('base', $this->base);
    }

    public function checkSharing(Request $request){
        $CollectionRate = null;
        if($request->subtitle){
            $CollectionRate = CollectionRate::where('col_acct_subtitle_id',$request->subtitle)->first();
        }else{
            $CollectionRate = CollectionRate::where('col_acct_title_id',$request->account)->first();
        }
    ...
    public function per_acct_report2( Request $request ){
        ...
        if(isset($subtitle) && $subtitle !== '' && $subtitle !== 0){
            ...
            $receipts = Receipt::distinct('id')->join('col_receipt_items','col_receipt_items.col_receipt_id' ,'=','col_receipt.id')
                            ->where('col_receipt.report_date','>=' , $data['start_date']->format('Y-m-d') )
                            ->where('col_receipt.report_date','<=' , $data['end_date']->format('Y-m-d') )
                            ->where('col_receipt.is_printed', '=', '1')
                            ...
                            ->orderBy('col_receipt.updated_at', 'DESC') // 
                            ->get();
            ...
            $cash_div = CashDivision::distinct('id')->join('col_cash_division_items','col_cash_division_items.col_cash_division_id' ,'=','col_cash_division.id')
            ...
            $receipts = Receipt::join('col_receipt_items','col_receipt_items.col_receipt_id' ,'=','col_receipt.id')
                            ...
                            ->where('col_receipt.is_printed', '=', '1')
                            ->where('col_receipt.is_cancelled', '=', '0')
                            ->where('col_receipt_items.col_acct_title_id', '=', $account_idx)
                            ->groupby('col_receipt.serial_no')
                            ->orderBy('col_receipt.date_of_entry')
                            ->orderBy('col_receipt.updated_at', 'DESC') // 
                            ->get();
            ...
            $cash_div = CashDivision::distinct('id')->join('col_cash_division_items','col_cash_division_items.col_cash_division_id' ,'=','col_cash_division.id')
                            ...
                            ->where('col_cash_division.date_of_entry','>=' , $data['start_date']->format('Y-m-d') )
                            ->where('col_cash_division.date_of_entry','<=' , $data['end_date']->format('Y-m-d') )
                            ->where('col_cash_division_items.col_acct_title_id', '=', $account_idx)
                            ->orderBy('col_cash_division.date_of_entry')
                            ->get();
       }
        ...
        foreach ($receipts as $rkey => $receipt) {
            ...
            // $receiptss[]  = Receipt::where('serial_no','=',$receipt->serial_no)->orderBy('updated_at', 'DESC')->first();

            $receiptss[]  = Receipt::where('serial_no','=',$receipt->serial_no)->where('is_cancelled', '0')->orderBy('updated_at', 'DESC')->first();
        }
        ...
        if(isset($request['button_pdf_professional_tax'] )){
            ...
            if($request->input('account') == '55' ){
                $pdf = PDF::loadView('collection::pdf/per_account_report_v2_html_sef', $this->base);
            }else{
            ...
            return @$pdf->stream();
             ...
             if($request->input('account') == '55' ){
              return view('collection::html.per_account_report_v2_html_sef',$this->base)->with('base', $this->base);
            }else{
        ...
        foreach($receipts as $receipt) {
         ...
         foreach($cashdivs as $cashdiv) {
                    ...
                    foreach($cashdiv->items as $item) {
                            ...
                            if(!isset($cashdiv->barangay->name)){
                                continue;
                            }
