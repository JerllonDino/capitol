...
use Modules\Collection\Entities\{Customer,Receipt,ReceiptItems,CashDivision,CashDivisionItems,Municipality,Barangay,PreviousTaxType,MunicipalReceipt,F56TDARP, RptMunicipalExcel, RptMunicipalExcelItems, RptSefAdjustments};
...
class CustomerController extends Controller
{
    ...
    public function rpt_record_dt(Request $req, $isPaid) {
        ...
        // $tax_decs = DB::connection('mysql2')->select(DB::raw('
        //     select tax_dec_owner_info.id as owner_id,
        //     tax_dec_archive_info.id as taxdec_id,
        //     tax_dec_no,
        //     address,
        //     type_o,
        //     municipality,
        ...
        //     and tax_dec_no != "" 
        //     and tax_dec_no is not null 
        //     and tax_dec_owner_info.deleted_at is null 
        //     and tax_dec_archive_info.deleted_at is null 
        //     and tax_dec_loc_property.deleted_at is null group by tax_dec_no'));
        
        $tax_decs = DB::table('capitol_rpt.tax_dec_archive_info as rpt_archive_info')
                        ->select('capitol_rpt.tax_dec_owner_info.id as owner_id', 
                                    ...
                                    'capitol.col_receipt.id as receipt_id'
                                    )
                        ->leftJoin('capitol.col_f56_tdarp', 'capitol.col_f56_tdarp.tdarpno', '=', 'rpt_archive_info.tax_dec_no')
        ...
        foreach($tax_decs as $det) {
            ...
            $tax_dec = $det->tax_dec_no;

            if($det->type_o == 'Spouse') {
                $get_owner_info = DB::connection('mysql2')->select(DB::raw('select spouse, married_to from tax_dec_archive_owner_spouse where owner_id = '.$det->owner_id . ' group by spouse, married_to'));
                $get_owner_info = collect($get_owner_info);
                $owners = [];
                foreach($get_owner_info as $info) {
                    $temp_owner = $info->spouse . ' & ' . $info->married_to;
                    if(!in_array($temp_owner, $owners)){
                        $owner_name .= nl2br($temp_owner. '
                        ');
                ...
                $owners = [];
                foreach($get_owner_info as $info) {
                    $temp_owner = $info->fname . ' ' . $info->mname . ' ' . $info->lname . ' ' . $info->ename;
                    if(!in_array($temp_owner, $owners)){
                        $owner_name .= nl2br($temp_owner .'
                        ');
                        array_push($owners, $temp_owner);
                ...
                $owners = [];
                foreach($get_owner_info as $info) {
                    if(!in_array($info->company_name, $owners)){
                ...
                $owners = [];
                foreach($get_owner_info as $info) {
                    $temp_owner = $info->owner_name . ' married to ' . $info->married_to;
                    if(!in_array($temp_owner, $owners)){
                        $owner_name .= nl2br($temp_owner .'
                        ');
                        array_push($owners, $temp_owner);
                ...
                $get_owner_info = DB::connection('mysql2')->select(DB::raw('select special_name from tax_dec_archive_owner_special where owner_id = '.$det->owner_id . ' group by special_name'));
                $get_owner_info = collect($get_owner_info);
                $owners = [];
                foreach($get_owner_info as $info) {
                    if(!in_array($info->special_name, $owners)){
                        $owner_name .= nl2br($info->special_name.'
                        ');
                        array_push($owners, $info->special_name);
                    }
        ...
        // $old_tax_decs = DB::connection('mysql2')->select(DB::raw('select 
        //     owner_name, 
        //     id, 
        //     municipality, 
        //     brgy, 
        ...
        $old_tax_decs = DB::table('capitol_rpt.tax_dec_old_archive_info as old_archive_info')
                            ->select('old_archive_info.owner_name', 
                                    'old_archive_info.id', 
                                    'old_archive_info.municipality', 
                                    'old_archive_info.brgy', 
                                    'old_archive_info.tax_dec_no',
                                    'old_archive_info.prevs_tax_dec_no',
                                    'old_archive_info.class',
                                    'old_archive_info.assessed_value'
                                    )
                            ->leftJoin('capitol.col_f56_tdarp', 'capitol.col_f56_tdarp.tdarpno', '=', 'old_archive_info.tax_dec_no')
                            ->leftJoin('capitol.col_f56_detail', 'capitol.col_f56_detail.id', '=', 'capitol.col_f56_tdarp.col_f56_detail_id')
                            ->leftJoin('capitol.col_receipt', 'capitol.col_receipt.id', '=', 'capitol.col_f56_detail.col_receipt_id')
                            ->when($isPaid == 1, function($query){
                                return $query->whereYear('col_receipt.report_date', '>=', '2019')
                            ...
                            ->when($isPaid == 0, function($query){
                                return $query->whereNull('capitol.col_receipt.id');
                            })
                            ->where([
                            ...
                            ->orWhere(function($query){
                                $query->where('old_archive_info.canceled_by_tdrp', '=', '')
                                        ->where('old_archive_info.canceled_by_tdrp', '=', null);
                            })->get();
    ...
    public function rpt_record_get($id = null, $td, $isPdf = null) {
        $id = $id == 'old' ? 'old' : Crypt::decrypt($id);
        $td = $id == 'old' ? $td : Crypt::decrypt($td);
        ...
        if(count($tax_decs) > 0) {
            ...
            foreach($tax_decs as $det) {
                ...
                if (!in_array($det->owner_id, $owners)) {
                    ...
                    array_push($owners, $det->owner_id);
                }
                    ...
                    foreach($split_td as $td_no) {
                        ...
                        if(count($getNewOwner) > 0) {
                            if($getNewOwner[0]->type_o == 'Spouse') {
                                $get_owner_info_new = DB::connection('mysql2')->select(DB::raw('select spouse, married_to from tax_dec_archive_owner_spouse where owner_id = '.$getNewOwner[0]->owner_id) . ' group by spouse, married_to');
                                $get_owner_info_new = collect($get_owner_info_new);
                                $owners = [];
                                foreach($get_owner_info_new as $info) {
                                    $temp_owner = $info->spouse . ' & ' . $info->married_to;
                                    if(!in_array($temp_owner, $owners)){
                                        $owner_new .= nl2br($temp_owner . '
                                        ');
                                        array_push($owners, $temp_owner);
                                    }
                                ...
                                $get_owner_info_new = DB::connection('mysql2')->select(DB::raw('select fname, lname, mname, ename from tax_dec_archive_owner_person where owner_id = '.$getNewOwner[0]->owner_id . ' group by fname, lname, mname, ename'));
                                $get_owner_info_new = collect($get_owner_info_new);
                                $owners = [];
                                foreach($get_owner_info_new as $info) {
                                    $temp_owner = $info->fname . ' ' . $info->mname . ' ' . $info->lname . ' ' . $info->ename;
                                ...
                                $get_owner_info_new = DB::connection('mysql2')->select(DB::raw('select company_name from tax_dec_archive_owner_company where owner_id = '.$getNewOwner[0]->owner_id . ' group by company_name'));
                                $get_owner_info_new = collect($get_owner_info_new);
                                $owners = [];
                                foreach($get_owner_info_new as $info) {
                                    if(!in_array($info->company_name, $owners)){
                                ...
                                $get_owner_info_new = DB::connection('mysql2')->select(DB::raw('select owner_name, married_to from tax_dec_archive_owner_marriedto where owner_id = '.$getNewOwner[0]->owner_id) . ' group by owner_name, married_to');
                                $get_owner_info_new = collect($get_owner_info_new);
                                $owners = [];
                                foreach($get_owner_info_new as $info) {
                                    $temp_owner = $info->owner_name . ' married to ' . $info->married_to;
                                ...
                                $get_owner_info_new = DB::connection('mysql2')->select(DB::raw('select special_name from tax_dec_archive_owner_special where owner_id = '.$getNewOwner[0]->owner_id) . ' group by special_name');
                                $get_owner_info_new = collect($get_owner_info_new);
                                $owners = [];
                                foreach($get_owner_info_new as $info) {
                                    if(!in_array($info->special_name, $owners)){
    ...
    private function getAndMergeImportedExcelPayments($tdarp, $payment_record)
    {
        ...
        foreach($excelPayments as $payment){
            ...
            $period_covered = explode('-', $payment->period_covered);
            if(end($period_covered) < $year){
                continue;
            }
            
            if(count($period_covered) > 1){
                if(end($period_covered) >= $year){
                    for ($i=$year; $i <= end($period_covered); $i++) { 
                        $payment_record[$payment->id][$payment->tdarp_number][$payment->or_number]['tax_type'] = [5];
                        $payment_record[$payment->id][$payment->tdarp_number][$payment->or_number]['discount'] = [( $i != $year ? $payment->basic_adv_discount : $payment->basic_current_discount)];
                        $payment_record[$payment->id][$payment->tdarp_number][$payment->or_number]['penalty'] = [$payment->basic_penalty_current];
                        $payment_record[$payment->id][$payment->tdarp_number][$payment->or_number]['tax'] = [($i != $year ? $payment->basic_adv_gross : $payment->basic_current_gross)];
                        $payment_record[$payment->id][$payment->tdarp_number][$payment->or_number]['period_covered'] = [$i];
