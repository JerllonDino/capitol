...
class CustomerController extends Controller
{
    ...
    // public function rpt_record($id, $mnth, $yr, Request $req) {
    // public function rpt_record($id, Request $req) {
    //     // if($mnth == 'month') {
    ...
    //             ->whereHas('F56Detailmny.TDARPX', function($q) use($req) {
    //                 $q->where('tdarpno', '=', $req->arp_no);
    //             })
    ...
    //         //     ->where('is_cancelled', 0)
    //         //     ->where('is_printed', 1)
    //         //     ->whereMonth('report_date', '=', $mnth)
    //         //     ->whereYear('report_date', '=', $yr)
    //         //     ->where('af_type', 2)
    //         //     ->get();
    //     // }

    //     // if(count($receipts) == 0) {
    //     //     Session::flash('error', ['No payment records found for ARP No. '.$req->arp_no]);
    ...
    //             $getNewOwner = DB::connection('mysql2')->select(DB::raw('select owner_id, type_o, effectivity_assesment_yr from tax_dec_archive_info join tax_dec_owner_info on tax_dec_archive_info.owner_id = tax_dec_owner_info.id where tax_dec_no = "'.$det->canceled_by_tdrp.'"'));  
    //             $owner_new = '';
    //             $effectivity_new = '';
    ...
    //                     $effectivity_new = $getNewOwner[0]->effectivity_assesment_yr;
    //                 } else {
    //                     $getNewOwner = DB::connection('mysql2')->select(DB::raw('select owner_name from tax_dec_old_archive_info where tax_dec_no = '.$det->canceled_by_tdrp));
    //                     if(count($getNewOwner) > 0) {
    ...
    //             $get_cancelBy = DB::connection('mysql2')->select(DB::raw('select id from tax_dec_archive_info where tax_dec_no = "'.$det->canceled_by_tdrp.'"'));
    //             if(count($get_cancelBy) == 0) {
    //                 $get_cancelBy = DB::connection('mysql2')->select(DB::raw('select id from tax_dec_old_archive_info where tax_dec_no = '.$det->canceled_by_tdrp.'"'));
    //             }
    ...
    //                 'cancelled_by' => $det->canceled_by_tdrp,
    //                 'owner_new' => $owner_new,
    //                 'effectivity_new' => $effectivity_new,
    //                 'id' => $get_cancelBy[0]->id,
    //             ]);
    //         }
    //     } else {
    //         $old_tax_decs = DB::connection('mysql2')->select(DB::raw('select owner_name, id, municipality, brgy, tax_dec_no, prevs_tax_dec_no, class, assessed_value, canceled_by_tdrp
    ...
    //         }
    //     }  
    //     // if($receipt_det == 0 || count($receipts) == 0) { // no RPT transactions..
    //     //     Session::flash('error', ['No records found for ARP No. '.$req->arp_no]);
    //     //     return redirect()->route('rpt.records_index');
    //     // }
    ...
    //     $prev_tax_dec_record = [];
    //     $prev_tax_dec_paymnt = [];
    //     $get_tax_dec = DB::connection('mysql2')->select(DB::raw('select prevs_tax_dec_no from tax_dec_archive_info where tax_dec_no = "'.$req->arp_no.'"')); 
    //     if(count($get_tax_dec) > 0) {
    //         // some tax decs have multiple previous tax decs, encoded using splecial characters and "AND"
    //         $clean = preg_replace("/[^0-9-\s]+/", " ", $get_tax_dec[0]->prevs_tax_dec_no);
    //         $split = explode(" ", $clean);
    //         for($i = 0; $i < count($split); $i++) {
    //             if($split[$i] != "") {
    //                 $owner_name = '';
    //                 $prev_tax_dec = DB::connection('mysql2')->select(DB::raw('select tax_dec_owner_info.id as owner_id, tax_dec_archive_info.id as taxdec_id, tax_dec_no, address, type_o, municipality, brgy, other_details, cert_title, class, tax_dec_archive_kind_class.assessed_value, actual_use  
    //                     from tax_dec_archive_info 
    //                     join tax_dec_owner_info on tax_dec_archive_info.owner_id = tax_dec_owner_info.id 
    //                     left join tax_dec_loc_property on tax_dec_loc_property.tax_dec_id = tax_dec_archive_info.id 
    //                     join tax_dec_archive_kind_class on tax_dec_archive_info.id = tax_dec_archive_kind_class.tax_dec_id
    ...
    //                             // $prev_tax_dec_paymnt[$prev_pay->col_customer_id][$prev_pay->serial_no]['date'] = $prev_pay->date_of_entry;
    //                             // $prev_tax_dec_paymnt[$prev_pay->col_customer_id][$prev_pay->serial_no]['period_covered'] = $prev_pay->period_covered;
    //                             // if($prev_pay->basic_previous > 0) {
    //                             //     $prev_tax_dec_paymnt[$prev_pay->col_customer_id][$prev_pay->serial_no]['tax'] = $prev_pay->basic_previous;
    //                             // } elseif($prev_pay->basic_current > 0) {
    //                             //     $prev_tax_dec_paymnt[$prev_pay->col_customer_id][$prev_pay->serial_no]['tax'] = $prev_pay->basic_current;
    //                             // }
    //                             // $prev_tax_dec_paymnt[$prev_pay->col_customer_id][$prev_pay->serial_no]['penalty'] = $prev_pay['basic_penalty_current'] + $prev_pay['basic_penalty_previous'];
    //                             // $prev_tax_dec_paymnt[$prev_pay->col_customer_id][$prev_pay->serial_no]['discount'] = $prev_pay['discount'];
    //                             // $prev_tax_dec_paymnt[$prev_pay->col_customer_id][$prev_pay->serial_no]['tax_type'] = $prev_pay->F56Detail->TDARPX['previous_tax_type_id'];
    ...
    //         $get_tax_dec = DB::connection('mysql2')->select(DB::raw('select prevs_tax_dec_no from tax_dec_old_archive_info where tax_dec_no = "'.$req->arp_no.'"')); 
    //         $clean = preg_replace("/[^0-9-\s]+/", " ", $get_tax_dec[0]->prevs_tax_dec_no);
    //         $split = explode(" ", $clean);
    //         for($i = 0; $i < count($split); $i++) {
    //             if($split[$i] != "") {
