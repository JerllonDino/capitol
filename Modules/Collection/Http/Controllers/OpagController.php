...
use Modules\Collection\Entities\{CollectionRate,Municipality,Customer,SandGravelTypes as sg_types, OpagCollection, OpagItems, Barangay};
...
class OpagController extends Controller
{
    ...
    {
        $this->base['page_title'] = 'OPAG Collections';
        ...
        return view('collection::opag.index')->with('base', $this->base);
    }
    ...
    public function show($id)
    {
        ...
        $this->base['addtl'] = OpagCollection::whereId($id)->first();
    ...
    {
        $addtl = OpagCollection::whereId($id)->first();
        ...
        $validator = Validator::make($request->all(), $filter);
        if ($validator->fails()) {
            return redirect()->route('opag.edit', ['id' => $id])
                ->withErrors($validator);
        } elseif (in_array('', $request['account_id'])) {
            $validator->getMessageBag()
                ->add('account', 'An account field is empty or not identified');
    ...
    public function opag_delete(Request $request){
        $opag = OpagCollection::find($request->input('opag'));
        $opag->delete();
        return response()->json('test');
    }

    public function opag_restore(Request $request){
        $opag = OpagCollection::withTrashed()->find($request->input('opag'));
        $opag->restore();
        return response()->json('test');
    }
